...
tests = describe "Glow.Gerbil.Fresh" $ do
  it ("Should generate fresh names when given duplicate names") $ do
    ...
    runState (traverse fresh ["a", "a", "a", "a"]) Map.empty `shouldBe` (["a","a0","a1","a2"], Map.fromList [("a",[3])])
  it ("Should not break on names with 0 at the end") $ do
    runState (fresh "a0") Map.empty `shouldBe` ("a0", Map.fromList [("a",[1])])
    runState (traverse fresh ["a", "a0", "a", "a0"]) Map.empty `shouldBe` (["a","a0","a1","a2"], Map.fromList [("a",[3])])
    runState (traverse fresh ["a0", "a", "a0", "a"]) Map.empty `shouldBe` (["a0","a1","a2","a3"], Map.fromList [("a",[4])])
  it ("Should prioritize given numbered names when not taken") $ do
    ...
    runState (traverse fresh ["a5", "a", "a", "a"]) Map.empty `shouldBe` (["a5","a0","a1","a2"], Map.fromList [("a",[3,4,6])])
